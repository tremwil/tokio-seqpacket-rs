...
/// have the same ancillary ownership semantics as their `recv` variants.
///
...
impl UnixSeqpacket {
	...
	pub fn poll_recv(&self, cx: &mut Context, buffer: &mut [u8]) -> Poll<std::io::Result<usize>> {
		self.poll_recv_internal(cx, buffer, false)
	}
	...
	pub fn poll_peek(&self, cx: &mut Context, buffer: &mut [u8]) -> Poll<std::io::Result<usize>> {
		self.poll_recv_internal(cx, buffer, true)
	}

	/// Shared implementation of poll_recv and poll_peek.
	fn poll_recv_internal(&self, cx: &mut Context, buffer: &mut [u8], peek: bool) -> Poll<std::io::Result<usize>> {
		loop {
			let mut ready_guard = ready!(self.io.poll_read_ready(cx)?);
			match ready_guard.try_io(|inner| sys::recv(inner.get_ref(), buffer, peek)) {
				Ok(result) => return Poll::Ready(result),
				Err(_would_block) => continue,
			}
		}
	...
	/// For that reason, it is preferable to use the async functions rather than polling functions when possible.
	pub fn poll_peek_vectored(&self, cx: &mut Context, buffer: &mut [IoSliceMut]) -> Poll<std::io::Result<usize>> {
	...
	pub fn poll_recv_with_ancillary<'a>(
		...
		self.poll_recv_vectored_with_ancillary_internal(cx, &mut [IoSliceMut::new(buffer)], ancillary_buffer, false)
	}
	...
	pub fn poll_peek_with_ancillary<'a>(
		...
		self.poll_recv_vectored_with_ancillary_internal(cx, &mut [IoSliceMut::new(buffer)], ancillary_buffer, true)
	}
	...
	pub async fn recv(&self, buffer: &mut [u8]) -> std::io::Result<usize> {
		self.recv_internal(buffer, false).await
	}
	...
	pub async fn peek(&self, buffer: &mut [u8]) -> std::io::Result<usize> {
		self.recv_internal(buffer, true).await
	}

	/// Shared implementation of `recv` and `peek`.
	async fn recv_internal(&self, buffer: &mut [u8], peek: bool) -> std::io::Result<usize> {
		loop {
	...
	/// All calling tasks will try to complete the asynchronous action,
	/// although the order in which they complete is not guaranteed.
	pub async fn peek_vectored(&self, buffer: &mut [IoSliceMut<'_>]) -> std::io::Result<usize> {
	...
	pub async fn recv_with_ancillary<'a>(
		...
		self.recv_vectored_with_ancillary_internal(&mut [IoSliceMut::new(buffer)], ancillary_buffer, false)
			.await
	}
	...
	pub async fn peek_with_ancillary<'a>(
		...
		self.recv_vectored_with_ancillary_internal(&mut [IoSliceMut::new(buffer)], ancillary_buffer, true)
			.await
	}
